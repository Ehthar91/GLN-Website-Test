...
let lessonImportScriptPromises=new Map();
...
function lessonImportLoadScript(src,ready){
  if(ready?.())return Promise.resolve();
  if(lessonImportScriptPromises.has(src))return lessonImportScriptPromises.get(src);
  const promise=new Promise((resolve,reject)=>{
    const script=document.createElement('script');script.src=src;script.async=true;
    script.onload=()=>ready?.()?resolve():reject(new Error('Reader loaded but did not initialize.'));
    script.onerror=()=>reject(new Error('Could not load the document reader. Check your internet connection.'));
    document.head.append(script);
  });
  lessonImportScriptPromises.set(src,promise);return promise;
}
...
async function lessonImportExtractPdf(file){
  await lessonImportLoadScript('https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.min.js',()=>Boolean(window.pdfjsLib));
  window.pdfjsLib.GlobalWorkerOptions.workerSrc='https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.worker.min.js';
  const data=new Uint8Array(await file.arrayBuffer());const pdf=await window.pdfjsLib.getDocument({data}).promise;const pages=[];
  for(let pageNumber=1;pageNumber<=pdf.numPages;pageNumber++){
    const page=await pdf.getPage(pageNumber);const content=await page.getTextContent();const groups=[];
    content.items.forEach(item=>{const y=Number(item.transform?.[5]||0);const x=Number(item.transform?.[4]||0);let group=groups.find(entry=>Math.abs(entry.y-y)<2);if(!group){group={y,items:[]};groups.push(group)}group.items.push({x,text:String(item.str||'')})});
    groups.sort((a,b)=>b.y-a.y);pages.push(groups.map(group=>group.items.sort((a,b)=>a.x-b.x).map(item=>item.text).join(' ').replace(/\s+/g,' ').trim()).filter(Boolean).join('\n'));
  }
  return pages.join('\n');
}
async function lessonImportExtractDocx(file){
  await lessonImportLoadScript('https://cdn.jsdelivr.net/npm/mammoth@1.8.0/mammoth.browser.min.js',()=>Boolean(window.mammoth));
...
async function lessonImportExtractFile(file){
  ...
  if(ext==='docx'||file.type==='application/vnd.openxmlformats-officedocument.wordprocessingml.document')return lessonImportExtractDocx(file);
  if(ext==='pdf'||file.type==='application/pdf')return lessonImportExtractPdf(file);
  throw new Error('Please choose a TXT, DOCX, or PDF lesson plan.');
}
