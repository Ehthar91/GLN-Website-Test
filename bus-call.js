...
(() => {
  ...
  function renderHistory(history={}){const list=q('#busCallHistory');const items=Object.values(history||{}).filter(Boolean).sort((a,b)=>(b.calledAt||0)-(a.calledAt||0)).slice(0,20);list.innerHTML='';if(!items.length){const p=document.createElement('p');p.textContent='No buses called yet.';list.append(p);return}items.forEach(item=>{const row=document.createElement('div');row.className='bus-history-row';const bus=document.createElement('b');bus.textContent=item.number||'—';const detail=document.createElement('span');detail.className='bus-history-detail';const stage=document.createElement('strong');stage.className=`bus-history-stage ${callStageClass(item.stage)}`;stage.textContent=callStageLabel(item.stage);detail.append(stage);if(item.note){const note=document.createElement('em');note.textContent=item.note;detail.append(note)}const time=document.createElement('time');time.textContent=formatTime(item.calledAt);row.append(bus,detail,time);list.append(row)})}
  function renderCaller(value){const activeAt=lastCallerActivity(value);q('#busCodeReservation').textContent=activeAt?`Code reserved until ${new Date(activeAt+RESERVATION_MS).toLocaleDateString()}. Using caller controls renews it for 30 days.`:'This code is reserved for its caller.';const listeners=Object.values(value.players||{}).filter(item=>item?.connected!==false);q('#busListenerCount').textContent=`${listeners.length} classroom${listeners.length===1?'':'s'} online`;renderHistory(value.history);if(value.currentCall){q('#busCallerStatus').textContent=`${callStageLabel(value.currentCall.stage)} for Bus ${value.currentCall.number} was sent at ${formatTime(value.currentCall.calledAt)}.`}else q('#busCallerStatus').textContent='Ready to call a bus.'}
  ...
  function showPopup(call,{test=false}={}){if(!call)return;const stage=callStageClass(call.stage);popupStage.textContent=callStageLabel(call.stage).toUpperCase();popupStage.className=`bus-popup-stage ${stage}`;popupNumber.textContent=cleanText(call.number,18)||'123';popupNumber.style.setProperty('--bus-digits',Math.max(3,popupNumber.textContent.length));const note=cleanText(call.note,80);popupNote.textContent=note;popupNote.hidden=!note;popupTime.textContent=test?'Test alert':`${callStageLabel(call.stage)} • Called at ${formatTime(call.calledAt)}`;popup.hidden=false;if(!test)beep()}
  function renderTeacher(value){const latest=value.currentCall;q('#busBoardNumber').textContent=latest?cleanText(latest.number,18):'—';q('#busBoardLabel').textContent=latest?`${callStageLabel(latest.stage).toUpperCase()} • LATEST BUS`:'WAITING FOR THE NEXT BUS';q('#busBoardNumber').style.setProperty('--bus-digits',Math.max(3,String(latest?.number||'').length));q('#busTeacherConnectionText').textContent='Connected';const call=value.currentCall;if(!call){popup.hidden=true;q('#busWaitingTitle').textContent='Waiting for the next bus';q('#busWaitingDetail').textContent='A large alert will appear on this screen when the caller sends a bus number.';return}if(String(call.id)!==lastCallId){lastCallId=String(call.id);showPopup(call);q('#busWaitingTitle').textContent=`${callStageLabel(call.stage)}: Bus ${call.number}`;q('#busWaitingDetail').textContent=`Called at ${formatTime(call.calledAt)}${call.note?` • ${call.note}`:''}`}}
