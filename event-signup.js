...
function activityNameForEvent(event,participants){
  ...
  if(event.activityMode==='event')return event.title||'Conference';
  if(event.activityMode==='event-participant')return names.length?`${event.title||'Conference'} – ${names.join(', ')}`:(event.title||'Conference');
  return names.join(', ')||(event.title||'Conference');
}
function syncEventToSpecialSchedule(id,event){
  ...
  for(const date of desiredDates){const daySlots=eventSlotsByDate(event).get(date)||[];const slots=daySlots.map(slot=>{const participants=eventSignupsForSlot(event,slot.id);if(!participants.length)return null;return{id:`event-${id}-${slot.id}`,name:activityNameForEvent(event,participants),time:slot.time,duration:Math.max(1,Math.min(480,Number(slot.duration)||15))}}).filter(Boolean).sort((a,b)=>a.time.localeCompare(b.time));
    ...
    const existing=index>=0?classroomSchedules[index]:null;const next={id:existing?.id||`event-special-${id}-${date}`,type:'special',source:'event-signup',linkedEventId:id,linkedEventDate:date,name:String(event.title||'Conference Sign-Up'),date,enabled:true,slots};
...
function syncSharedEditorSchedule(id,event){
  ...
  for(const date of desiredDates){const daySlots=groups.get(date)||[];const slots=daySlots.map(slot=>{const participants=eventSignupsForSlot(event,slot.id);if(!participants.length)return null;return{id:`shared-event-${id}-${slot.id}`,name:activityNameForEvent(event,participants),time:slot.time,duration:Math.max(1,Math.min(480,Number(slot.duration)||15))}}).filter(Boolean).sort((a,b)=>a.time.localeCompare(b.time));
    let index=classroomSchedules.findIndex(item=>item.type==='special'&&item.source==='event-signup-shared-editor'&&item.linkedEventId===id&&(item.linkedEventDate===date||(!item.linkedEventDate&&item.date===date)));const existing=index>=0?classroomSchedules[index]:null;const next={id:existing?.id||`shared-event-special-${id}-${date}-${Date.now()}`,type:'special',source:'event-signup-shared-editor',linkedEventId:id,linkedEventDate:date,name:String(event.title||'Conference Sign-Up'),date,enabled:true,slots};if(index>=0)classroomSchedules[index]=next;else{classroomSchedules.push(next);index=classroomSchedules.length-1}touched.add(next.id);
  }
...
function toggleSharedEditorSchedule(){
  if(!sharedEditorEventId||!sharedEditorEventData)return;if(sharedEditorScheduleLinked(sharedEditorEventId)){classroomSchedules=classroomSchedules.filter(item=>!(item.type==='special'&&item.source==='event-signup-shared-editor'&&item.linkedEventId===sharedEditorEventId));saveClassroomSchedules();renderScheduleTimers();updateScheduleTimerClock();updateSharedScheduleButton();sharedEditorMessage('Removed all days of this conference from your Schedule Timer.','ok');return}
  for(const date of eventDates(sharedEditorEventData)){classroomSchedules.push({id:`shared-event-special-${sharedEditorEventId}-${date}-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,type:'special',source:'event-signup-shared-editor',linkedEventId:sharedEditorEventId,linkedEventDate:date,name:String(sharedEditorEventData.title||'Conference Sign-Up'),date,enabled:true,slots:[]})}syncSharedEditorSchedule(sharedEditorEventId,sharedEditorEventData);updateSharedScheduleButton();sharedEditorMessage('Linked every conference day to your Schedule Timer. Reservation changes will keep each Special Schedule updated while this editor link is open.','ok')
}
...
function subscribeTeacherEvent(id){
  ...
  },error=>setEventStatus(error?.code==='PERMISSION_DENIED'?'Conference Sign-Up needs the included Firebase database rules before shared conferences can load.':(error?.message||'Could not load Conference Sign-Up.'),'error'));
...
async function refreshEventSignupManager(){
  ...
  try{await initRaceFirebase();eventSignupIds.forEach(subscribeTeacherEvent);setEventStatus(eventSignupIds.length?'Conferences are live. Reservation changes will appear automatically.':'Ready. Create a conference to get a public sign-up link.','ok');renderEventSignupList()}
  catch(error){setEventStatus(error?.message||'Could not connect to Conference Sign-Up.','error')}
}
async function saveEventFromForm(event){
  ...
  if(!title||!defaultDate){alert('Enter a conference name and first conference date.');return}
  if(!slots.length||slots.length!==eventSignupEditorSlots.length){alert('Add a date and start time for every conference slot.');return}
  ...
  try{
    ...
    eventSignupCache.set(id,data);subscribeTeacherEvent(id);syncEventToSpecialSchedule(id,data);renderEventSignupList();resetEventSignupEditor();setEventStatus(`Saved “${title}” across ${eventDates(data).length} day${eventDates(data).length===1?'':'s'}.`,'ok');showToast('Conference saved');
  }catch(error){setEventStatus(error?.code==='PERMISSION_DENIED'?'Firebase blocked Conference Sign-Up. Publish the current Realtime Database rules included with this build.':(error?.message||'Could not save the conference.'),'error')}
}
async function deleteEventSignup(id,event){
  if(!confirm(`Delete ${event.title||'this conference'} and all of its conference reservations?`))return;
  try{await initRaceFirebase();await fb.remove(fb.ref(db,`eventSignups/${id}`));eventSignupIds=eventSignupIds.filter(item=>item!==id);saveEventSignupIds();eventSignupCache.delete(id);eventSignupWatchers.get(id)?.();eventSignupWatchers.delete(id);removeLinkedEventSchedule(id);renderEventSignupList();if(editingEventSignupId===id)resetEventSignupEditor();showToast('Conference deleted')}
  catch(error){setEventStatus(error?.message||'Could not delete the conference.','error')}
}
...
function renderSharedEditor(){
  const event=sharedEditorEventData;if(!event||!eventSharedEditorSlots)return;eventSharedEditorTitle.textContent=event.title||'Conference Sign-Up';eventSharedEditorDate.textContent=eventDateSummary(event);eventSharedEditorDescription.textContent=event.description||'Manage conference reservations for this conference.';eventSharedEditorSlots.innerHTML='';
...
async function openSharedEventEditor(id,token){
  ...
  try{await initRaceFirebase();if(!currentUser)throw new Error('Firebase sign-in is not ready.');const base=`eventSignups/${id}`;await fb.set(fb.ref(db,`${base}/editorSessions/${currentUser.uid}`),{token,createdAt:Date.now(),lastUsedAt:Date.now()});const snap=await fb.get(fb.ref(db,base));if(!snap.exists())throw new Error('This conference no longer exists.');sharedEditorEventData=snap.val();renderSharedEditor();sharedEditorMessage('Editor access verified. You can view conference reservations, move students to open times, and remove reservations. Only the conference owner can edit or delete the conference.','ok');sharedEditorUnsubscribe=fb.onValue(fb.ref(db,base),next=>{if(!next.exists()){sharedEditorMessage('This conference was deleted by the owner.','error');sharedEditorEventData=null;eventSharedEditorSlots.innerHTML='';return}sharedEditorEventData=next.val();renderSharedEditor();syncSharedEditorSchedule(id,sharedEditorEventData)},error=>sharedEditorMessage(error?.code==='PERMISSION_DENIED'?'This editor link was revoked or is no longer valid.':(error?.message||'Editor access was lost.'),'error'))}
...
function renderPublicSelection(){
  ...
  if(eventPublicSubmit){eventPublicSubmit.textContent=publicManageMode?'Save changes':(slots.length>1?`Save ${slots.length} conference times`:'Save conference time');eventPublicSubmit.disabled=!slots.length}
}
function renderPublicEvent(){
  const event=publicEventData;if(!event)return;initializePublicSelection();eventPublicTitle.textContent=event.title||'Conference Sign-Up';eventPublicDate.textContent=eventDateSummary(event);eventPublicDescription.textContent=event.description||'Choose an available time slot below.';
  ...
  if(publicManageMode){publicMessage(own.length?'Private management link verified. You can change or cancel only this conference reservation.':'This private management link has no active appointments. You can choose a new time if the conference is still open.',own.length?'ok':'')}
  else if(event.status==='closed')publicMessage('This conference is closed for new reservations.','error');else publicMessage(event.allowMultiple===false?'Choose one time slot, then enter the parent/guardian and student information.':'Choose one or more time slots. Parent/guardian information is entered once; each time gets its own student details.');
...
async function claimEventSpot(slot,token){
  const spotIds=eventSpotIds(slot);if(!spotIds.length)throw new Error('This conference needs to be updated by the organizer before it can accept new reservations.');let lastPermission=null;
...
async function savePublicSignup(event){
  ...
  try{
    ...
    if(publicManageMode){applyPrivateReservationData(Object.fromEntries(nextRecords));renderPublicEvent();publicMessage('Your conference changes were saved.','ok')}
...
async function openPublicEvent(id,manageToken=''){
  ...
  try{
    ...
    if(!titleSnap.exists()){eventPublicTitle.textContent='Conference not found';eventPublicSlots.innerHTML='';publicMessage('This conference sign-up link is no longer available.','error');return}
  ...
  }catch(error){publicMessage(error?.code==='PERMISSION_DENIED'?'Conference Sign-Up needs the updated private-link Firebase database rules before the public link can open.':(error?.message||'Could not load this conference.'),'error')}
}
